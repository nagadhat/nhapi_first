...
class UserCustomerRepository implements UserCustomerRepositoryInterface
{
    ...
    {
        $user = $this->user::leftjoin('user_customers', 'users.username', '=', 'user_customers.username')
            ->where('users.username', $userName)
            ->select('users.*', 'user_customers.first_name')
            ->first();
        $newUser = $user->toArray();

        $addresses = $this->address_assign::leftjoin('address', 'address_assign.address_id', '=', 'address.id')
